...
| `texture_filter` (inherited from CanvasItem) | enum | Now propagated to the plugin's internal draw items — set Nearest on the node for crisp pixel-art-style text/images without touching the project default. \"Inherit\" resolves to the project default (tree-walking inheritance doesn't reach RenderingServer-created items). |
